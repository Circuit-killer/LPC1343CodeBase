# 1 "drivers/displays/bitmap/sharpmem/sharpmem.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "drivers/displays/bitmap/sharpmem/sharpmem.c"
# 72 "drivers/displays/bitmap/sharpmem/sharpmem.c"
...
# 73 "drivers/displays/bitmap/sharpmem/sharpmem.c" 2

# 1 "drivers/displays/bitmap/sharpmem/sharpmem.h" 1
# 39 "drivers/displays/bitmap/sharpmem/sharpmem.h"
# 1 "./projectconfig.h" 1
# 40 "./projectconfig.h"
# 1 "./lpc134x.h" 1
# 46 "./lpc134x.h"
...
# 40 "drivers/displays/bitmap/sharpmem/sharpmem.h" 2
...
# 41 "drivers/displays/bitmap/sharpmem/sharpmem.h" 2
# 86 "drivers/displays/bitmap/sharpmem/sharpmem.h"
void sharpmemInit(void);
void sharpmemEnable(_Bool enable);
...
# 75 "drivers/displays/bitmap/sharpmem/sharpmem.c" 2
...
# 77 "drivers/displays/bitmap/sharpmem/sharpmem.c" 2
...
# 78 "drivers/displays/bitmap/sharpmem/sharpmem.c" 2




static uint8_t _sharpmembuffer[((96) * (96)) / 8];
static volatile uint8_t _sharpmem_vcom = (0x40);
# 95 "drivers/displays/bitmap/sharpmem/sharpmem.c"
...
void sharpmemSendByte(uint8_t data)
{
  ...
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(1)); } while(0);
  ...
  for (i=0; i<8; i++)
  {
    if (data & 0x80)
    {
      do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(2)); (*((REG32 *) (0x50023FFC))) |= (1<<(2)); } while(0);
    }
    else
    {
      do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(2)); } while(0);
    }

    do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(1)); (*((REG32 *) (0x50023FFC))) |= (1<<(1)); } while(0);
    data <<= 1;
    __asm volatile("nop");
    do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(1)); } while(0);
  }
...
void sharpmemInit(void)
{
  ...
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(1)); } while(0);
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(2)); } while(0);
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(3)); } while(0);
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(4)); } while(0);
...
# 221 "drivers/displays/bitmap/sharpmem/sharpmem.c"
void sharpmemEnable(_Bool enable)
{
  if (enable)
  {
    do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(4)); (*((REG32 *) (0x50023FFC))) |= (1<<(4)); } while(0);
  }
  else
  {
    do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(4)); } while(0);
...
# 243 "drivers/displays/bitmap/sharpmem/sharpmem.c"
void sharpmemDrawPixel(uint16_t x, uint16_t y)
{
  if ((x >= (96)) || (y >= (96)))
    return;

  _sharpmembuffer[(y*(96) + x) /8] |= (1 << x % 8);
}
# 261 "drivers/displays/bitmap/sharpmem/sharpmem.c"
void sharpmemClearPixel(uint16_t x, uint16_t y)
{
  if ((x >= (96)) || (y >= (96)))
    return;
...
# 281 "drivers/displays/bitmap/sharpmem/sharpmem.c"
uint8_t sharpmemGetPixel(uint16_t x, uint16_t y)
{
  if ((x >=(96)) || (y >=(96))) return 0;
  return _sharpmembuffer[(y*(96) + x) /8] & (1 << x % 8) ? 1 : 0;
}
...
void sharpmemClearScreen()
{
  memset(_sharpmembuffer, 0x00, ((96) * (96)) / 8);

  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(3)); (*((REG32 *) (0x50023FFC))) |= (1<<(3)); } while(0);
  sharpmemSendByte(_sharpmem_vcom | (0x20));
  sharpmemSendByte(0x00);
  do { _sharpmem_vcom = _sharpmem_vcom ? 0x00 : (0x40); } while(0);;
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(3)); } while(0);
}
...
void sharpmemRefresh(void)
{
  ...
  do { (*((REG32 *) (0x50023FFC))) &= ~(1<<(3)); (*((REG32 *) (0x50023FFC))) |= (1<<(3)); } while(0);
  sharpmemSendByte((0x80) | _sharpmem_vcom);
  do { _sharpmem_vcom = _sharpmem_vcom ? 0x00 : (0x40); } while(0);;
...
# 377 "drivers/displays/bitmap/sharpmem/sharpmem.c"
void sharpmemDrawString(uint16_t x, uint16_t y, const char* text, struct FONT_DEF font)
{
  uint8_t l;
