# 1 "core/uart/uart_buf.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "core/uart/uart_buf.c"
# 51 "core/uart/uart_buf.c"
...
uint8_t uartRxBufferRead()
{
  ...
  pcb->rxfifo.rd_ptr = (pcb->rxfifo.rd_ptr + 1) % (512);
  pcb->rxfifo.len--;
  return data;
}
...
# 111 "core/uart/uart_buf.c"
void uartRxBufferWrite(uint8_t data)
{
  uart_pcb_t *pcb = uartGetPCB();

  pcb->rxfifo.buf[pcb->rxfifo.wr_ptr] = data;
  pcb->rxfifo.wr_ptr = (pcb->rxfifo.wr_ptr + 1) % (512);
  pcb->rxfifo.len++;
}
