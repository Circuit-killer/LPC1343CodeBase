...
# 100 "./project/cmd_tbl.h"
void cmd_dmx(uint8_t argc, char **argv);
# 113 "./project/cmd_tbl.h"
cmd_t cmd_tbl[] =
{

  { "?", 0, 0, 0, cmd_help , "Help" , "This command has no parameters" },
  { "V", 0, 0, 0, cmd_sysinfo , "System Info" , "This command has no parameters" },
  { "Z", 0, 0, 0, cmd_reset , "Reset" , "This command has no parameters" },
...
void cmdParse(char *cmd)
{
  ...
  do
  {
      argv[++i] = strtok(((void *)0), " ");
  } while ((i < 30) && (argv[i] != ((void *)0)));
  ...
  for (i=0; i < (sizeof(cmd_tbl)/sizeof(cmd_t)); i++)
  {
      if (!strcmp(argv[0], cmd_tbl[i].command))
      {
        ...
        else if ((argc - 1) < cmd_tbl[i].minArgs)
        {
          ...
          printf ("Too few arguments (%d expected)%s", cmd_tbl[i].minArgs, "\r\n");
          printf ("%sType '%s ?' for more information%s%s", "\r\n", cmd_tbl[i].command, "\r\n", "\r\n");
        ...
        else if ((argc - 1) > cmd_tbl[i].maxArgs)
        {
          ...
          printf ("Too many arguments (%d maximum)%s", cmd_tbl[i].maxArgs, "\r\n");
          printf ("%sType '%s ?' for more information%s%s", "\r\n", cmd_tbl[i].command, "\r\n", "\r\n");
  ...
  printf("Command not recognized: '%s'%s%s", cmd, "\r\n", "\r\n");

  printf("Type '?' for a list of all available commands%s", "\r\n");
...
void cmd_help(uint8_t argc, char **argv)
{
  ...
  for (i=0; i < (sizeof(cmd_tbl)/sizeof(cmd_t)); i++)
  {
    if (!cmd_tbl[i].hidden)
    {
      printf ("%-10s   %s%s", cmd_tbl[i].command, cmd_tbl[i].description, "\r\n");
    }
  }
