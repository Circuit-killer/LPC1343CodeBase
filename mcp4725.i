# 1 "drivers/dac/mcp4725/mcp4725.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "drivers/dac/mcp4725/mcp4725.c"
# 56 "drivers/dac/mcp4725/mcp4725.c"
# 1 "drivers/dac/mcp4725/mcp4725.h" 1
# 40 "drivers/dac/mcp4725/mcp4725.h"
# 1 "./projectconfig.h" 1
# 40 "./projectconfig.h"
# 1 "./lpc134x.h" 1
# 46 "./lpc134x.h"
...
# 41 "drivers/dac/mcp4725/mcp4725.h" 2
...
int mcp4725Init();
void mcp4725SetVoltage( uint16_t output, _Bool writeEEPROM );
void mcp4725ReadConfig( uint8_t *status, uint16_t *value );
# 57 "drivers/dac/mcp4725/mcp4725.c" 2
# 1 "./core/i2c/i2c.h" 1
# 97 "./core/i2c/i2c.h"
...
# 58 "drivers/dac/mcp4725/mcp4725.c" 2

extern volatile uint8_t I2CMasterBuffer[64];
extern volatile uint8_t I2CSlaveBuffer[64];
extern volatile uint32_t I2CReadLength, I2CWriteLength;

static _Bool _mcp4725Initialised = 0;
...
int mcp4725Init()
{
  ...
  _mcp4725Initialised = 1;

  return 0;
}
# 100 "drivers/dac/mcp4725/mcp4725.c"
void mcp4725SetVoltage( uint16_t output, _Bool writeEEPROM )
{
  ...
  I2CWriteLength = 4;
  I2CReadLength = 0;
  I2CMasterBuffer[0] = (0xC4);
  if (writeEEPROM)
  {
    I2CMasterBuffer[1] = ((0x60));
  }
  else
  {
    I2CMasterBuffer[1] = ((0x40));
  }
  I2CMasterBuffer[2] = (output / 16);
  I2CMasterBuffer[3] = (output % 16) << 4;
  i2cEngine();
}
# 138 "drivers/dac/mcp4725/mcp4725.c"
void mcp4725ReadConfig( uint8_t *status, uint16_t *value )
{
  if (!_mcp4725Initialised) mcp4725Init();
